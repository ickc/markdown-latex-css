...
<!-- 
# Copyright (C) 2010-2012  Fletcher T. Penney <fletcher@fletcherpenney.net>
#
...
<xsl:stylesheet
	...
	<xsl:template match="/opml">
		...
		<xsl:apply-templates select="/opml/body/outline[last()][translate(@text,'ABCDEFGHIJKLMNOPQRSTUVWXYZ',
		'abcdefghijklmnopqrstuvwxyz') = 'metadata']" mode="meta"/>
	...
	<xsl:template match="/opml/body/outline[last()]">
		...
		<xsl:if test="not(translate(@text,'ABCDEFGHIJKLMNOPQRSTUVWXYZ',
			...
			<xsl:apply-templates select="outline" mode="meta"/>
					<xsl:value-of select="concat($header,' ')"/>
					<xsl:value-of select="@text"/>
					<xsl:value-of select="concat(' ',$header)"/>
					<xsl:text>
